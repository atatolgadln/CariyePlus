...
class BanContext extends Command {
	...
	async contextMenuRun(interaction) {
		...
		collector.on('collect', async i => {
			if (i.customId === 'confirm_ban') {
				await handleConfirmBan(interaction, i, a, interaction.guild);
			}
...
async function handleConfirmBan(interaction, i, a, guild) {
	try {
		const confirmationEmbed = new EmbedBuilder()
			.setColor('Random')
			.setTitle('Ban Successful')
			.setThumbnail('https://media.giphy.com/media/l3V0j3ytFyGHqiV7W/giphy.gif')
			.setDescription(`Mission 'Ban-${a.user.username}' has been succussful.`)
			.setTimestamp()
		...
		await guild.members.ban(a, `Banned by ${i.user.username}`);
	}
	catch (error) {
		console.error(error);
